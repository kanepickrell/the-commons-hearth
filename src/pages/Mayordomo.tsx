...
export default function Mayordomo() {
  ...
  const [actingWorkshop, setActingWorkshop] = useState<Record<string, 'approve' | 'reject' | null>>({});
  ...
  const loadWorkshopQueue = useCallback(async () => {
    const { data, error } = await supabase
      .from('workshops')
      .select(
        'id, title, description, craft, location_text, held_at, created_at, language, host:profiles!workshops_host_id_fkey(display_name), parish:parishes(name, city)'
      )
      .eq('status', 'pending')
      .order('created_at', { ascending: true });

    if (error) {
      console.error('Failed to load pending gatherings:', error);
      toast({
      ...
      setPendingWorkshops((data ?? []) as unknown as PendingWorkshop[]);
    }
  }, [t]);
  ...
  }, [authLoading, user, isAdmin, loadQueue, loadWorkshopQueue]);
  ...
  const actWorkshop = async (id: string, action: 'approve' | 'reject') => {
    setActingWorkshop((s) => ({ ...s, [id]: action }));
    const snapshot = pendingWorkshops;
    setPendingWorkshops((rows) => rows.filter((r) => r.id !== id));

    const rpc = action === 'approve' ? 'approve_workshop' : 'reject_workshop';
    const { error } = await supabase.rpc(rpc, { p_id: id });

    setActingWorkshop((s) => ({ ...s, [id]: null }));

    if (error) {
      console.error(`${rpc} failed:`, error);
      setPendingWorkshops(snapshot);
      toast({
        title:
          action === 'approve'
            ? t({ en: 'Could not approve', es: 'No pudimos aprobar' })
    ...
    toast({
      title:
        action === 'approve'
          ? t({ en: 'Gathering approved', es: 'Reunión aprobada' })
          : t({ en: 'Gathering rejected', es: 'Reunión rechazada' }),
    });
  };
  ...
  return (
    <Layout>
      <div className="container-wide py-12 md:py-16 space-y-16">
        {/* ------------------------------------------------------------------ */}
        {/* Section 1: Pending member approvals */}
        {/* ------------------------------------------------------------------ */}
        <section>
        ...
        <section className="border-t border-mesquite/10 pt-12">
          <h2 className="mb-2 font-heading text-3xl text-mesquite md:text-4xl">
            {t({ en: 'Pending gatherings', es: 'Reuniones pendientes' })}
          </h2>
          <p className="mb-10 font-serif text-lg italic text-mesquite/70">
            {t({
              en: 'Gatherings submitted by members. Approve to open them for RSVP.',
              es: 'Reuniones enviadas por los miembros. Aprueba para abrirlas a confirmaciones.',
            })}
          </p>
          ...
          {pendingWorkshops.length === 0 ? (
            ...
            <ul className="space-y-6">
              {pendingWorkshops.map((w) => {
                const currentAction = actingWorkshop[w.id];
                const busy = !!currentAction;
                return (
                  <li key={w.id} className="rounded-sm border border-mesquite/20 bg-cal/60 p-6">
                    <div className="mb-1 flex items-baseline justify-between gap-4">
                      <h3 className="font-heading text-2xl text-mesquite">{w.title}</h3>
                      <span className="font-mono text-xs text-mesquite/40">
                        {t({ en: 'submitted', es: 'enviada' })}{' '}
                    ...
                    <p className="mb-3 font-serif text-sm italic text-mesquite/70">
                      {t({ en: 'Hosted by', es: 'Ofrecido por' })}{' '}
                      {w.host?.display_name ?? '—'}
                      {w.parish ? ` · ${w.parish.name}` : ''}
                    </p>

                    <p className="mb-3 font-serif text-sm text-mesquite">
                      {new Date(w.held_at).toLocaleString(locale === 'es' ? 'es-MX' : 'en-US', {
                        weekday: 'short',
                        month: 'long',
                        day: 'numeric',
                        year: 'numeric',
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={() => actWorkshop(w.id, 'approve')}
                        disabled={busy}
                        className="inline-flex items-center gap-2 rounded-sm bg-ocre px-5 py-2 font-heading text-sm text-cal transition hover:bg-mesquite disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        {currentAction === 'approve'
                      ...
                      <button
                        onClick={() => actWorkshop(w.id, 'reject')}
                        disabled={busy}
                        className="inline-flex items-center gap-2 rounded-sm border border-mesquite/30 px-5 py-2 font-heading text-sm text-mesquite transition hover:border-rojo hover:text-rojo disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        {currentAction === 'reject'
